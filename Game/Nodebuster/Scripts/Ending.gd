...
func vanilla_1314004716__ready() -> void:
	credits.hide()
	virus_deployed_box.hide()

func vanilla_1314004716__process(delta: float) -> void:
	loading_circle.rotation_degrees -= 180*delta

func vanilla_1314004716_start_ending() -> void:
...
func vanilla_1314004716__set_static_vol(val: float) -> void:
...
func _ready():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1314004716__ready, [], 1937942880)
	else:
		vanilla_1314004716__ready()


func _process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1314004716__process, [delta], 3828194538)
	else:
		vanilla_1314004716__process(delta)


func start_ending():
	if _ModLoaderHooks.any_mod_hooked:
		await _ModLoaderHooks.call_hooks_async(vanilla_1314004716_start_ending, [], 219510926)
	else:
		await vanilla_1314004716_start_ending()


func _set_static_vol(val: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1314004716__set_static_vol, [val], 1487147502)
	else:
		vanilla_1314004716__set_static_vol(val)
