...
func vanilla_2307298626_set_color(_color: Color) -> void:
	color = _color
	modulate = color

func vanilla_2307298626_setup(_resource_type: int, _max_health: float, _damage: float, _movespeed: float,
_scale: float) -> void:
...
func vanilla_2307298626_set_move_dir(dir: Vector2) -> void:
	move_dir = dir

func vanilla_2307298626_get_width() -> float:
	return 128*scale.x

func vanilla_2307298626_take_damage(amount: float) -> void:
...
func vanilla_2307298626_pulse() -> void:
	if Globals.flashing:
...
func vanilla_2307298626__process(delta: float) -> void:
	if rainbow:
		modulate.h = fmod(modulate.h+delta, 1.0)

func vanilla_2307298626__physics_process(delta: float) -> void:
	time_alive += delta
...
func vanilla_2307298626_update_bar() -> void:
	texture_bar.value = curr_health

func vanilla_2307298626_set_dead() -> void:
	dead = true
...
func set_color(_color: Color):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_set_color, [_color], 563293452)
	else:
		vanilla_2307298626_set_color(_color)


func setup(_resource_type: int, _max_health: float, _damage: float, _movespeed: float, 
_scale: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_setup, [_resource_type, _max_health, _damage, _movespeed, _scale], 2688192435)
	else:
		vanilla_2307298626_setup(_resource_type, _max_health, _damage, _movespeed, _scale)


func set_move_dir(dir: Vector2):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_set_move_dir, [dir], 2257510338)
	else:
		vanilla_2307298626_set_move_dir(dir)


func get_width() -> float:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_2307298626_get_width, [], 2904842081)
	else:
		return vanilla_2307298626_get_width()


func take_damage(amount: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_take_damage, [amount], 1796604453)
	else:
		vanilla_2307298626_take_damage(amount)


func pulse():
	if _ModLoaderHooks.any_mod_hooked:
		await _ModLoaderHooks.call_hooks_async(vanilla_2307298626_pulse, [], 2685200875)
	else:
		await vanilla_2307298626_pulse()


func _process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626__process, [delta], 2437286208)
	else:
		vanilla_2307298626__process(delta)


func _physics_process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626__physics_process, [delta], 259281666)
	else:
		vanilla_2307298626__physics_process(delta)


func update_bar():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_update_bar, [], 1512659705)
	else:
		vanilla_2307298626_update_bar()


func set_dead():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_2307298626_set_dead, [], 3140706747)
	else:
		vanilla_2307298626_set_dead()
