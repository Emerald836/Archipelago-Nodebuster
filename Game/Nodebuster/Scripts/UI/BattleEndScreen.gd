...
func vanilla_187411703__ready() -> void:
	home_btn.pressed.connect(_on_home_pressed)
	restart_btn.pressed.connect(_on_restart_pressed)

# code: 0 if health zeroed. 1 if boss defeated. 2 if terminated by user.
func vanilla_187411703_setup(obtained_resources: Dictionary, code: int) -> void:
	if code == 0:
...
func vanilla_187411703__on_home_pressed() -> void:
	home_pressed.emit()

func vanilla_187411703__on_restart_pressed() -> void:
...
func _ready():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_187411703__ready, [], 3415167915)
	else:
		vanilla_187411703__ready()


func setup(obtained_resources: Dictionary, code: int):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_187411703_setup, [obtained_resources, code], 1037811848)
	else:
		vanilla_187411703_setup(obtained_resources, code)


func _on_home_pressed():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_187411703__on_home_pressed, [], 565046768)
	else:
		vanilla_187411703__on_home_pressed()


func _on_restart_pressed():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_187411703__on_restart_pressed, [], 885908940)
	else:
		vanilla_187411703__on_restart_pressed()
