...
func vanilla_1009365406__ready() -> void:
...
func vanilla_1009365406__process(delta: float) -> void:
	if dead: return
...
func vanilla_1009365406_attack() -> void:
	corner_box.pulse()
...
func vanilla_1009365406__physics_process(delta: float) -> void:
	if dead: return
...
func _ready():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1009365406__ready, [], 1665005970)
	else:
		vanilla_1009365406__ready()


func _process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1009365406__process, [delta], 2952642972)
	else:
		vanilla_1009365406__process(delta)


func attack():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1009365406_attack, [], 1746187606)
	else:
		vanilla_1009365406_attack()


func _physics_process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1009365406__physics_process, [delta], 1125951326)
	else:
		vanilla_1009365406__physics_process(delta)
