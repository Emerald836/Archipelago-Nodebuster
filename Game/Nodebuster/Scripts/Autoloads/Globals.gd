...
func vanilla_1709788310__ready() -> void:
	randomize()

func vanilla_1709788310__process(delta: float) -> void:
	mouse_pos = get_global_mouse_position()

var mouse_blocker: Control
var mouse_locks: Dictionary = {}
func vanilla_1709788310_lock_mouse(holder) -> void:
	mouse_locks[holder] = true
	mouse_blocker.show()
func vanilla_1709788310_unlock_mouse(holder) -> void:
	mouse_locks.erase(holder)
	mouse_blocker.visible = not mouse_locks.is_empty()
func vanilla_1709788310_is_mouse_locked() -> bool:
	return not mouse_locks.is_empty()


# If play_effects: do shake + pulse sfx
func vanilla_1709788310_create_explosion(pos: Vector2, dangle: float, width: float, damage_mod: float = 0.0,
...
func vanilla_1709788310_create_pulse_bolts(pos: Vector2) -> void:
...
func _ready():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310__ready, [], 3140742794)
	else:
		vanilla_1709788310__ready()


func _process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310__process, [delta], 3712275604)
	else:
		vanilla_1709788310__process(delta)


func lock_mouse(holder):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310_lock_mouse, [holder], 3981365959)
	else:
		vanilla_1709788310_lock_mouse(holder)


func unlock_mouse(holder):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310_unlock_mouse, [holder], 2533277578)
	else:
		vanilla_1709788310_unlock_mouse(holder)


func is_mouse_locked() -> bool:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_1709788310_is_mouse_locked, [], 3656796043)
	else:
		return vanilla_1709788310_is_mouse_locked()


func create_explosion(pos: Vector2, dangle: float, width: float, damage_mod: float=0.0, 
play_effects: bool=true):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310_create_explosion, [pos, dangle, width, damage_mod, play_effects], 1704777450)
	else:
		vanilla_1709788310_create_explosion(pos, dangle, width, damage_mod, play_effects)


func create_pulse_bolts(pos: Vector2):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_1709788310_create_pulse_bolts, [pos], 1643719989)
	else:
		vanilla_1709788310_create_pulse_bolts(pos)
