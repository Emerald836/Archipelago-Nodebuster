...
func vanilla_587862972__ready() -> void:
	process_mode = Node.PROCESS_MODE_ALWAYS

func vanilla_587862972__process(delta: float) -> void:
...
func vanilla_587862972_scale(target: Object, force: float, x: float = 1, stiffness: float = 250,
...
func vanilla_587862972_create(x: float = 1, stiffness: float = 150, dampening: float = 10) -> Spring:
	var spr: Spring = Spring.new(x, stiffness, dampening)
	springs_list.append(spr)
	
	return spr

func vanilla_587862972_spring_property(target: Object, property: NodePath, force: float,
...
func vanilla_587862972_rotate(target: Node, force: float, x: float = 0, stiffness: float = 600,
...
func vanilla_587862972_squash(target: Object, x_force: float, y_force: float, x: float = 1,
...
func vanilla_587862972_cleanup() -> void:
	scale_springs.clear()
	springs_list.clear()
...
func _ready():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_587862972__ready, [], 3266509360)
	else:
		vanilla_587862972__ready()


func _process(delta: float):
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_587862972__process, [delta], 3233112506)
	else:
		vanilla_587862972__process(delta)


func scale(target: Object, force: float, x: float=1, stiffness: float=250, 
dampening: float=12) -> Spring:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_587862972_scale, [target, force, x, stiffness, dampening], 2855321508)
	else:
		return vanilla_587862972_scale(target, force, x, stiffness, dampening)


func create(x: float=1, stiffness: float=150, dampening: float=10) -> Spring:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_587862972_create, [x, stiffness, dampening], 3423051440)
	else:
		return vanilla_587862972_create(x, stiffness, dampening)


func spring_property(target: Object, property: NodePath, force: float, 
x: float=0, stiffness: float=150, dampening: float=10) -> Spring:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_587862972_spring_property, [target, property, force, x, stiffness, dampening], 2944770931)
	else:
		return vanilla_587862972_spring_property(target, property, force, x, stiffness, dampening)


func rotate(target: Node, force: float, x: float=0, stiffness: float=600, 
dampening: float=14) -> Spring:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_587862972_rotate, [target, force, x, stiffness, dampening], 4007063627)
	else:
		return vanilla_587862972_rotate(target, force, x, stiffness, dampening)


func squash(target: Object, x_force: float, y_force: float, x: float=1, 
stiffness: float=250, dampening: float=12) -> Array[Spring]:
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(vanilla_587862972_squash, [target, x_force, y_force, x, stiffness, dampening], 4048606769)
	else:
		return vanilla_587862972_squash(target, x_force, y_force, x, stiffness, dampening)


func cleanup():
	if _ModLoaderHooks.any_mod_hooked:
		_ModLoaderHooks.call_hooks(vanilla_587862972_cleanup, [], 1056729572)
	else:
		vanilla_587862972_cleanup()
